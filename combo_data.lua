...
local function save_combo()
    if combo_finished and combo_start.p1 and combo_start.p2 and combo_finish.p1 and combo_finish.p2 then
        local combo_data = {
            ...
            totals = {
                damage = (combo_start.p2.current_HP or 0) - (combo_finish.p2.current_HP or 0),
                p1_drive_gain = (combo_finish.p1.drive or 0) - (combo_start.p1.drive or 0),
                p1_super_gain = (combo_finish.p1.super or 0) - (combo_start.p1.super or 0),
                p2_drive_gain = (combo_finish.p2.drive or 0) - (combo_start.p2.drive or 0),
...
local function check_combo_started()
    if p2.curr_hitstun and p2.curr_hitstun > 0 and not combo_started then
        return true
    end
    return false
end
...
local function check_combo_finished()
    if combo_started and not combo_finished then
        if p2.curr_hitstun and p2.curr_hitstun == 0 and (p2.stance == 0 or p2.stance == 1) then
            return true
        elseif p2.stance and p2.stance ~= 0 and p2.stance ~= 1 and p2.full_invuln == 1 then
            return true
        end
    end
    return false
    ...
    if imgui.tree_node("Current Combo") then
        if combo_started or combo_finished then
            if imgui.begin_table("current_combo_table", 10) then
                ...
                imgui.table_setup_column("P1Pos", nil, 20)
                ...
                imgui.text(tostring(combo_start.p1.drive or ""))
                imgui.table_set_column_index(3)
                imgui.text(tostring(combo_start.p1.super or ""))
                imgui.table_set_column_index(4)
                imgui.text(tostring(combo_start.p2.drive or ""))
                ...
                if combo_start.p2.posX then
                    ...
                    if not combo_started then
                        ...
                        imgui.table_set_column_index(2)
                        imgui.text(tostring(combo_finish.p1.drive or ""))
                        imgui.table_set_column_index(3)
                        imgui.text(tostring(combo_finish.p1.super or ""))
                        imgui.table_set_column_index(4)
                        imgui.text(tostring(combo_finish.p2.drive or ""))
                        imgui.table_set_column_index(5)
                ...
                if combo_finished then
                    ...
                    imgui.table_set_column_index(2)
                    local finished_p1_drive_diff = (combo_finish.p1.drive or 0) - (combo_start.p1.drive or 0)
                    ...
                    imgui.table_set_column_index(4)
                    local finished_p2_drive = (combo_finish.p2.drive or 0) - (combo_start.p2.drive or 0)
                    ...
                    imgui.table_set_column_index(7)
                    local finished_position_change = (combo_finish.p1.posX or 0) - (combo_start.p1.posX or 0)
                    color(finished_position_change)

                    imgui.table_set_column_index(8)
                    local carry = (combo_finish.p2.posX or 0) - (combo_start.p2.posX or 0)
                    color(carry)
    ...
    if show_saved_combos then
        ...
        if imgui.tree_node(string.format("Saved Combos (%d)", #all_combos)) then
            ...
            if #all_combos > 0 then
                ...
                if imgui.begin_table("saved_combos_table", 11, table_flags) then
                    ...
                    for i, combo in ipairs(all_combos) do
                        ...
                        imgui.table_set_column_index(2)
                        local drive_gain = combo.totals.p1_drive_gain or 0
                        color(drive_gain)
                        
                        imgui.table_set_column_index(3)
                        local super_gain = combo.totals.p1_super_gain or 0
                        color(super_gain)
                        
                        imgui.table_set_column_index(4)
                        local p2_drive_loss = combo.totals.p2_drive_gain or 0
                        color(p2_drive_loss)
                        
                        imgui.table_set_column_index(5)
                        local p2_super_loss = combo.totals.p2_super_gain or 0
                        color(p2_super_loss)
                        ...
                        if imgui.begin_popup("combo_details_popup_" .. i) then
                            ...
                            if imgui.begin_table("combo_details_table_" .. i, 2, 0) then
                                ...
                                imgui.table_set_column_index(1)
                                imgui.text(string.format("%.0f", combo.start.p1.drive or 0))
                                
                                imgui.table_next_row()
                                imgui.table_set_column_index(0)
                                imgui.text("Finish P1 Drive:")
                                imgui.table_set_column_index(1)
                                imgui.text(string.format("%.0f", combo.finish.p1.drive or 0))
                                ...
                                imgui.table_set_column_index(1)
                                imgui.text(string.format("%.0f", combo.start.p2.drive or 0))

                                imgui.table_next_row()
                                imgui.table_set_column_index(0)
                                imgui.text("Finish P2 Drive:")
                                imgui.table_set_column_index(1)
                                imgui.text(string.format("%.0f", combo.finish.p2.drive or 0))
